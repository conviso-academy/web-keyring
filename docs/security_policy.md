...
A presente política se aplica a todas as camadas utilizadas na aplicação, sendo elas o front-end, back-end/API, banco de dados e camada de persistência, infraestrutura de aplicação, serviços auxiliares, mecanismos de autenticação, criptografia e qualquer futura integração na pipeline.
...
- **4.7** As decisões de autorização deverão ser realizadas no lado do servidor, não sendo permitido confiar em informações fornecidas pelo cliente para determinar privilégios de acesso.
...
- **8.6** Os logs deverão possuir política de retenção definida, considerando requisitos operacionais, legais e de segurança.
...
- **9.2** Todo pull-request deverá passar por uma revisão por pares dos envolvidos no desenvolvimento.
...
- **9.4** Após toda modificação significativa no projeto, deverá ser realizado um scan de vulnerabilidades, documentado na Conviso Platform.
- **9.5** Vulnerabilidades identificadas deverão possuir tratamento documentado, contendo ao menos sua classificação, responsável, prazo para correção e situação atual.
- **9.6** Não deverá ser realizado deploy de alterações que contenham vulnerabilidades críticas conhecidas sem uma justificativa formal e aprovação do responsável pelo projeto.
- **9.7** Dados de produção não deverão ser utilizados em ambientes de desenvolvimento ou testes sem a aplicação dos controles necessários de anonimização ou proteção.
- **9.8** O código deverá ser desenvolvido considerando as principais vulnerabilidades de aplicações web segundo o OWASP Top 10.
...
- **11.1** O tratamento de dados pessoais realizado pela aplicação deverá observar a legislação aplicável, especialmente a Lei Geral de Proteção de Dados Pessoais (LGPD).
- **11.2** As medidas de segurança deverão ser consideradas desde a fase de concepção da aplicação até sua execução e manutenção.
- **11.3** A aplicação deverá limitar a coleta e retenção de dados pessoais ao mínimo necessário para seu funcionamento.
- **11.4** O acesso a dados pessoais deverá ser restrito aos usuários e componentes que possuam necessidade legítima de acesso.

---

## 12. Revisão da Política

- **12.1** Esta política deverá ser revisada periodicamente ou sempre que houver alteração relevante na arquitetura, nas funcionalidades ou no modelo de ameaça da aplicação.
- **12.2** Alterações nesta política deverão possuir controle de versão e autoria identificada.
- **12.3** Novas funcionalidades que envolvam autenticação, autorização, criptografia, compartilhamento ou processamento de secrets deverão ser avaliadas quanto ao impacto de segurança antes de sua implementação em produção.